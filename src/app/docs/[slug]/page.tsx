  ...
  const { ref, inView } = useInView({ threshold: 0 });
   ...
   useEffect(() => {
    async function fetchDoc() {
      setLoading(true);
      const fetchedDoc = await getDocumentBySlugAction(params.slug);
      setDoc(fetchedDoc);
      if (fetchedDoc) {
          const headings = Array.from(fetchedDoc.content.matchAll(/^(#{2,4})\s+(.*)/gm)).map(match => {
  ...
  if (!doc) {
    return (
      <div className="container mx-auto max-w-4xl px-4 py-8 text-center">
          ...
          <h1 className="font-headline text-3xl">Document Not Found</h1>
          <p className="text-muted-foreground mt-2">The document you are looking for does not exist.</p>
